...
def truncated_svd_gesdd(
    ...
    """
    reg = torch.as_tensor(ad_decomp_reg, dtype=M.real.dtype if M.is_complex() else M.dtype, device=M.device)
    ...
    def v1_11_backward(self, gU, gS, gVh):
        ...
        if is_complex:
            ...
            if not torch.allclose(imdiag_UhgU, -imdiag_VhgV, 1e-2, 1e-2):
                warnings.warn(
                    "svd_backward: The singular vectors in the complex case are "
        ...
        gA = (UhgU * S.unsqueeze(-2) + S.unsqueeze(-1) * VhgV) * (1.0 / reg_preinv(E)) + gS.diag_embed()
        if is_complex:
            gA = gA + (UhgU.diagonal(0, -2, -1) * (1.0 / (2.0 * reg_preinv(S)))).diag_embed()
